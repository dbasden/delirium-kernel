...
#define getHunk getpage
#define releaseHunk freepage
...
static struct master_index *static_master_index;

void setup_pools() {
	INIT_SEMAPHORE(pool_mutex);
	SPIN_WAIT_SEMAPHORE(pool_mutex);
	static_master_index = create_master_index(NULL);
...
void * pool_alloc(int exponent) {
	void *ptr;
	SPIN_WAIT_SEMAPHORE(pool_mutex); {
		ptr = palloc(static_master_index, exponent);
	} RELEASE_SEMAPHORE(pool_mutex);
...
void pool_free(int exponent, void *ptr) {
	SPIN_WAIT_SEMAPHORE(pool_mutex); {
		pfree(static_master_index, exponent, ptr);
	} RELEASE_SEMAPHORE(pool_mutex);
}
